...
- Lastig om extra queries in het schema te defineren voor filteren of uitbreiden van de resultaten
- 

## Gebrek aan documentatie
Alleerst het gebrek aan documentatie

## Niet Naleven van SOLID-Principes
Een groot probleem van het AllesOnline CMS is het gebrek aan naleving van de SOLID-principes. Deze vijf principes zijn belangrijk voor het ontwikkelen van goed gestructureerde, uitbreidbare en onderhoudbare software. Het negeren van deze principes leidt vaak tot complexe en starre code, wat het toevoegen van nieuwe functionaliteiten bemoeilijkt en onderhoudsproblemen veroorzaakt.

### Voorbeeld: Ontbrekende Functionaliteit
Een concreet voorbeeld van de gevolgen van het niet naleven van de SOLID-principes is de **FormTagsModule**. Deze module maakt het mogelijk om relaties te definiëren tussen entiteiten, zoals one-to-many of many-to-many relaties. Echter, momenteel ontbreekt de functionaliteit voor het definiëren van een **reverseRelation**.

In scenario's waarin relaties tussen personen gedefinieerd moeten worden, kan een **BelongsToMany**-relatie met een **pivot**-table worden gebruikt. Dit stelt ons in staat om de koppelingen tussen entiteiten op te slaan en de relatie bidirectioneel te benaderen. Bij zelfrefererende relaties, zoals tussen personen van dezelfde klasse, wordt een **reverseRelation** gebruikt om redundantie te voorkomen. In dit geval beheert één entiteit de **foreignPivotKey** en de andere de **relatedPivotKey**, waardoor beide entiteiten eenvoudig hun gekoppelde relaties kunnen benaderen zonder duplicatie in de database.

### Problemen van het Niet Naleven van SOLID
Om deze functionaliteit toch beschikbaar te stellen, moeten ontwikkelaars de **FormTagsModule** uitbreiden of een aangepaste module creëren. Vaak kiezen ze voor de tweede optie, waarbij ze een groot deel van de bestaande module dupliceren en aanpassen. Dit komt voornamelijk doordat de code niet voldoet aan de SOLID-principes.

- **Single Responsibility Principle (SRP)**: Veel modules binnen het CMS hebben meerdere verantwoordelijkheden. De **FormTagsModule** is bijvoorbeeld verantwoordelijk voor tagbeheer, modelrelaties, configuratiebeheer en gegevensverwerking. Deze overvolle structuur maakt de module onoverzichtelijk en moeilijk aanpasbaar, wat de kans op fouten vergroot bij het toevoegen van nieuwe functionaliteiten zoals een **reverseRelation**.
    
- **Open/Closed Principle (OCP)**: De code is niet ontworpen voor eenvoudige uitbreiding zonder bestaande code aan te passen. Dit maakt het moeilijk om nieuwe functionaliteiten toe te voegen zonder de bestaande implementatie te wijzigen, wat de kans op regressies vergroot en de complexiteit verhoogt.
    
- **Code Duplication**: Door het ontbreken van functionaliteitenzijn ontwikkelaars vaak gedwongen om bestaande modules te dupliceren. Dit leidt tot onnodige code duplicatie, zoals in het geval van de **FormTagsModule**, waar secties worden gekopieerd en aangepast. Dit maakt het systeem moeilijker te onderhouden en vergroot de kans op inconsistenties in de codebase.
    

Het niet naleven van SOLID-principes heeft dus gevolgen voor de flexibiliteit en onderhoudbaarheid van het AllesOnline CMS. Het dwingt ontwikkelaars tot inefficiënte oplossingen zoals het dupliceren van code. Dit heeft geleidt tot een fragiele codebase die moeilijk aan te passen en uit te breiden is, wat de kwaliteit en toekomstbestendigheid van het systeem onder druk zet.